...
## Stocked Quadrat Sampling Method {.font90}
...
- **Density Variance**:
  $$
  \hat{V}(\hat{\lambda}) = \frac{1}{n a^2} \left(e^{\hat{\lambda} a} - 1\right)
  $$
- **Population Variance**:
  $$
  \hat{V}(\hat{M}) = A^2 \hat{V}(\hat{\lambda}) = \frac{A^2}{n a^2} \left(e^{\hat{\lambda} a} - 1\right)
  $$
:::
...
where $X = \frac{y}{n}$ and $E(X) = e^{-\lambda a}$. Calculating the derivative $g'(x)$:
...
$$
\hat{V}(X) = \frac{E(X)(1 - E(X))}{n} = \frac{e^{-\lambda a}(1 - e^{-\lambda a})}{n}
$$

Substituting back into the delta method formula:

$$
\hat{V}(\hat{\lambda}) = \left(-\frac{1}{a} e^{\lambda a}\right)^2 \cdot \frac{e^{-\lambda a}(1 - e^{-\lambda a})}{n} = \frac{1}{n a^2} \left(e^{\lambda a} - 1\right)
$$
...
## Introduction to Adaptive Sampling {.font80}
