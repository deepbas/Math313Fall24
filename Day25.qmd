...
=======
========
Systematic sampling offers several advantages, but its effectiveness hinges on two key factors:

- **Order of the sampling units:** The population must not have periodic patterns that align with the sampling interval.
- **Randomness of the starting point:** A truly random starting point ensures that the entire population is well-represented.
>>>>>>>> origin/main:Day25.qmd

However, there are potential issues:

- **Cyclic behavior:** If the population exhibits periodic patterns, it can introduce bias.
- **Difficulty in error estimation:** Estimating unbiased sampling errors is challenging, as results depend heavily on a single starting point.

Recognizing these trade-offs is crucial for the effective use of systematic sampling in practice.
...
We will now compare the variances obtained in parts (a) and (b) with simple random sampling of sizes $n=4$ and $n=8$.
